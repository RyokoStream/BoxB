...
function rollEpsilon() {
    ...
    let color = (epsilon % 2 !== 0) ? '赤' : '黄色';
    document.getElementById('epsilonResult').innerText = 'εの結果: ' + epsilon + ' そして色は ' + color;
    totalCount += c;

    // 色に基づいて赤と黄色の個数を計算
    let redCount = color === '赤' ? c : 40 - c;
    let yellowCount = color === '黄色' ? c : 40 - c;

    // 最終結果を表示
    document.getElementById('finalResult').innerText = '箱Bの赤は ' + redCount + ' 個、黄色は ' + yellowCount + ' 個になりました。';
